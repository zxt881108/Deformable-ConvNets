...
def predict_single_url(cfg,predictor,single_url,nms,box_voting,idx,all_boxes,jsonlistwithscore ,jsonlistwithoutscore,
                       thresh, vis=False, use_box_voting=False):
    """
...
def demo_net(cfg,predictor_list, dataset, image_set,
    ...
    for idx in range(0,num_images):
        single_url = image_set_index[idx]
 #       josnwithoutscore , jsonwithscore ,  = predict_single_url(predictor_list,single_url,nms,box_voting,idx,all_boxes,
 #                                                                jsonlistwithscore,jsonlistwithscore,
 #                                                                thresh, vis=False, use_box_voting=False)


        pool.apply_async(predict_single_url, (predictor_list[idx%3],single_url,
                                              nms,box_voting,idx,all_boxes,jsonlistwithscore,jsonlistwithscore,
                                              thresh, vis , use_box_voting,))
    pool.join()
    pool.close()
...
def demo_rfcn(cfg, dataset, image_set, root_path, dataset_path,
    ...
    for idx in idxlist:
        ctx = [mx.gpu(int(idx))]
        predictor_list.append(load_rfcn_model(cfg,has_rpn,prefix,epoch,ctx))
